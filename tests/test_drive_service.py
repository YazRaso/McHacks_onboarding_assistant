...
class TestDriveService:
    ...
    async def test_process_new_document(
        ...
        # Setup mocks
        mock_lookup_client.return_value = {'client_id': 'test', 'api_key': 'encrypted_key'}
        ...
        drive_service.get_file_metadata = Mock(return_value={
            'id': '123',
            'name': 'Test Doc',
            'modifiedTime': '2026-01-12T10:00:00Z',
        ...
        args = mock_create_doc.call_args[1] if mock_create_doc.call_args[1] else mock_create_doc.call_args[0]
        assert 'Test content' in str(args)
    
    @pytest.mark.asyncio
    @patch('src.backend.db.lookup_drive_document')
    async def test_process_unchanged_document(
        self,
        mock_lookup_doc,
        drive_service
    ):
